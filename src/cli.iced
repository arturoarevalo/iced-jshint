...
hintFiles = require "./hint"
{argv, help} = require("optimist")
	.usage('$0 [options] filename.iced ...')
	.options
		options:
			alias: 'o'
...
switch
	...
	when argv.help then console.log help()
		...
		# Filter out non-iced-coffee paths
		{iced, other} = _(argv._).groupBy (path) -> switch
			when /.+\.iced$/.test path then "iced"
			when /.+\.coffee$/.test path then "iced"
			else "other"

		if argv.verbose and other?.length > 0
			console.log "Skipping files that don't end in .iced or .coffee:\n" + other.join('\n')

		errors = hintFiles(iced,
